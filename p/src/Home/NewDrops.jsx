...
function NewDrops() {
  ...
  const handleWishlist = (item, isInWishlist) => {
    ...
    if (isInWishlist) {
      ...
      addWishlistItem({ ...item, productId: item.id, userId: user.id }).catch(console.error);
  ...
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return products.filter((item) => item.newArrival === true);
    },
  });
            ...
            return (
              ...
              >
                ...
                <div className="h-[280px] w-full bg-[#fcfaf7] flex items-center justify-center overflow-hidden relative border-b border-stone-100">
                  ...
                  {/* Wishlist Glassmorphic Floating Button */}
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
